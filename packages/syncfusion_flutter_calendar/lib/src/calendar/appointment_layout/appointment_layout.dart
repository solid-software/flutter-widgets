...
class _AppointmentLayoutState extends State<AppointmentLayout> {
  ...
  void _updateDayAppointmentDetails(
    ...
    for (int i = 0; i < _appointmentCollection.length; i++) {
      ...
      const double outerPadding = 3.0;
      const double innerGapHalf = 0.5;

      double finalX = xPosition;
      double finalWidth = appointmentWidth;

      if (appointmentView.position == 0) {
        finalX += outerPadding;
        finalWidth -= outerPadding;
      } else {
        finalX += innerGapHalf;
        finalWidth -= innerGapHalf;
      }

      if (appointmentView.position + 1 >= appointmentView.maxPositions) {
        finalWidth -= outerPadding;
      } else {
        finalWidth -= innerGapHalf;
      }
